...
for row in $(jq -r '.Certificates[] | @base64' $CONFIG); do
  ...
  latest_cert=$(jq -r '.Certificates[] | select(.Domain.Main == "'$domain'").Certificate' $acme_file | base64 -d | openssl x509 -noout -dates | grep notAfter)
  ...
  if [[ "$latest_cert" != "$current_cert" ]]; then
    ...
    > $keyfile
    > $certfile
    jq -r '.Certificates[] | select(.Domain.Main == "'$domain'").Key' $acme_file \
      | base64 -d > $keyfile

    jq -r '.Certificates[] | select(.Domain.Main == "'$domain'").Certificate' $acme_file \
      | base64 -d >> $certfile
